...
## Override vs Overload:
    Override는 상속은 클래스의 함수를 고쳐서 쓰고 싶을때(ex: Student 과제에서 toString()함수) @Override를 함수위에 붙여줌으로 함수를 덮어쓴다는 개념이고
    Overload는 같은 클래스 안에서 함수명이 같은데 매개변수의 유형과 개수를 다르게 가지게하여 다양한 유형의 호출에 대해 각기 다른 함수를 사용하게 하는 기능이다 

## System.exit()
    현제 running하고 있는 프로그램을 강제 종료 시키는 함수. System의 exit() 함수는 어떤 값을 매개변수로 받아도 종료된다 한다. 또 일반적으로 정상적으로 종료시에는 System.exit(0)을 사용하고 비정상 종료시에는 System.exit(1)을 사용한다 한다.
    참조:
